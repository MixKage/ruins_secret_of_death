...
- Каждые 50 этажей появляется отдельный босс — дочь некроманта.
...
### Урон врагов и броня
```text
reduced_portion = attack * 0.7 - armor * (1 - enemy.armor_pierce)
bypass_portion = attack * 0.3
damage = max(1, round(reduced_portion + bypass_portion))
```
