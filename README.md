...
- Комнаты между этажами: источник благодати (полный хил; после 50 — малое зелье), сундук древних (шанс награды на +2 этажа, можно экипировать/оставить; даёт малое зелье, после 50 — ещё среднее), костер паломника (+2-3 к макс. HP; даёт малое зелье, после 50 — ещё среднее).
...
- Для Странника руин, Охотника и Палача состояние «на последнем издыхании» даёт точность 100% (HP ≤ 1/3 от максимального).
...
### Решимость (Странник руин, полное здоровье)
```text
final_damage *= 1.2
```
- Активна у Странника руин, когда HP == hp_max.
...
- Опыт начисляется за завершенные забеги (этажи + сокровища) и за сезонные награды.
...
- Решимость (Странник руин, полное здоровье) усиливает урон магии на 20%.
