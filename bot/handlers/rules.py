...
def _format_badges() -> str:
    lines = [
        "<b>Значки</b>",
        "<i>Сезонные значки сохраняются в истории и могут стакаться (x2 и т.д.).</i>",
        "",
    ]
...
def _format_seasons() -> str:
    return "\n".join(
        [
            ...
            "Сезонные значки сохраняются навсегда и отображаются в истории.",
        ]
    )
...
def _format_enemies() -> str:
    lines = [
        ...
        "Элиты (50+): <b>Проклятый Клинок тени</b> и <b>Проклятая Каменная шкура</b>.",
        "",
    ]
    for item in ENEMIES:
